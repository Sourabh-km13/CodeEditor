export default function handleTerminalCreation  (container, ws )  {
    console.log('handle terminal creation');
...
function processStreamOutput(stream, ws) {
    ...
    function processStreamData(data) {
        ...
        if(data) {
            buffer = Buffer.concat([buffer, data]); // Concatenating the incoming
            //  data to the buffer
        }

        if(!nextDataType) {
            // If the next data type is not known, then we need to read the next
            //  8 bytes to determine the type and length of the message
            if(buffer.length >= 8) {
                const header = bufferSlicer(8);
                nextDataType = header.readUInt32BE(0); // The first 4 bytes 
                // represent the type of the message
                nextDataLength = header.readUInt32BE(4); // The next 4 bytes 
                // represent the length of the message

                processStreamData(); // Recursively call the function to process
                //  the message
            }
        } else {
            if(buffer.length >= nextDataLength) {
                const content = bufferSlicer(nextDataLength); // Slice the buffer 
